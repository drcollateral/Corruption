...
import { setPlayers } from "../core/GameState.js";
import { CLASSES, CLASS_LIST, getClassByName } from "../data/Classes.js";
import { spellsWithMeta, passivesFor, spellsFor } from "../data/SpellRegistry.js";

const BASE = 8;
const MIN = 8, MAX = 16;

export function wireCharacterCreate(targetCount, onAllDone){
  ...
  form.addEventListener("submit", (e) => {
    e.preventDefault();
    // PATCH: Remove blocking check for unspent points
    // if (updatePoints() !== 0) return;
    const attrs = currentAttrs();
    // Auto-generate a flavorful default name if user leaves blank
    const autoName = (() => {
      const aff = affinitySel.value;
      const cls = currentClass.name;
      // Simple epithet pools per class/affinity theme
      const epithets = {
        Elementalist: {
          Flame: ['Kindleborn','Cinder Adept','Emberweaver','Ashcaller'],
          Terra: ['Stoneward','Earthshaper','Bedrock Sage','Gravemender'],
          Tide:  ['Mistbinder','Brine Scribe','Wavecaller','Foam Whisper'],
        },
        Warrior: {
          Brutality: ['Skullbreaker','Red Edge','Furyhand','Iron Hewn'],
          Guardianship: ['Bulwark','Shieldroot','Wardwatch','Aegis Bearer'],
          Leadership: ['Bannerheart','Field Marshal','Linecaller','Rallyborn'],
        },
        Survivalist: {
          Lethality: ['Silent Fang','Night Arrow','Shade Tracker','Quickthorn'],
          Primalism: ['Feral Voice','Wild Pulse','Totemkin','Howl Singer'],
          "Nature's Aid": ['Green Hand','Bloomwarden','Sap Surgeon','Verdant Thread'],
        }
      };
      const list = (epithets[cls] && epithets[cls][aff]) || ['Pathfinder'];
      const pick = list[Math.floor(Math.random()*list.length)];
      // Format: Class Affinity epithet (omit affinity if redundant)
      if (cls === 'Elementalist') return `${aff} ${pick}`; // affinity shows element
      return `${cls} ${pick}`;
    })();
    const draft = {
      name: nameInput.value.trim() || autoName,
