...
import { spellsWithMeta, passivesFor } from "../data/SpellRegistry.js";
...
export function renderCharacterSheet(id){
  if (!state.ui) state.ui = {}; state.ui.rightDock = 'sheet';
  const p = findById(id);
...
export function renderBag(id){
  if (!state.ui) state.ui = {}; state.ui.rightDock = 'bag';
  const p = findById(id);
...
export function renderSpellbook(id){
  if (!state.ui) state.ui = {}; state.ui.rightDock = 'spells';
  const p = findById(id);
